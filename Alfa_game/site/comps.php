...
<head>
	...
	<link rel="stylesheet" href="css/compss.css">
</head>
<body>
	<div class="main_grid">
		<header>
		...
		<main>
			<div class="txt">
				...
				<p>Игровые компьютеры с индивидуальной сборкой, которые создаются<br> специально для вас, под любой бюджет и с лучшей ценой. Используем<br> только надежные комплектующие, гарантируем качество нашей<br> продукции и предоставляем техническую поддержку.<br>Каждый компьютер имеет конфигурации на выбор, а также конфигуратор<br> с проверкой совместимости – вы легко можете настроить сборку под<br> свои предпочтения.<br>Мы обеспечиваем быструю и надежную доставку вашего нового<br> компьютера в любую точку страны.</p>
			</div>

			<div class="comp-img"></div>

			<div class="red_btn"><a href="#comps"><p>Выбрать</p><a></div>

			<a name="comps"><h2>Модели игровых компьютеров</h2></a>
			<div class="parent">
			<?php

				$products = mysqli_query($link, query:"SELECT *  FROM `products` WHERE `category_id` = '1'"); /* Выборка элементов из базы данных */
				$products = mysqli_fetch_all($products);
				foreach ($products as $products) {
			?>
				<div class="div">
					<h3>GeForce GTX 1650</h3>
					<p>Игровой компьютер на</p>
					<img class="img" src="css/img_comps/<?= $products[8] ?>.png" alt="">

					<h2><?= $products[1] ?></h2>
					<p class="p"><?= $products[2] ?></p>
					<p class="price">От <?= $products[3] ?> Р</p>
					<button><a href="detail_comps.php?id=<?= $products[0] ?>">Подробнее</a></button>
				</div>
			...
			<div class="parent-2">
				...
				<div class="block">
					<a href=""><h3>Компактные<br>Игровые компьютеры</h3></a>
					<p>Серия компактных, но мощных<br> игровых компьютеров </p>
				</div>
				<div class="block">
					<a href=""><h3>Профессиональные<br> игровые компьютеры</h3></a>
					<p>Серия профессиональных компьютеров для<br> представителей творческих профессий</p>
				</div>
				<div class="block">
					<a href=""><h3>Игровые компьютеры<br> Poweres by MSI</h3></a>
