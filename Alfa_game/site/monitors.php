...
<head>
	...
	<link rel="stylesheet" href="css/monitorss.css">
</head>
<body>
	<div class="main_grid">
		<header>
		...
		<main>
			...
			<h2>Модели игровых мониторов</h2>

			<div class="parent">
				<?php

					$products = mysqli_query($link, query:"SELECT *  FROM `products` WHERE `category_id` = '3'"); /* Выборка элементов из базы данных */
					$products = mysqli_fetch_all($products);
					foreach ($products as $products) {
				?>
				<div class="div">
					<p>Игровой монитор</p>
					<img class="img" src="css/img_monitors/<?= $products[8] ?>.png" alt="">

					<h2><?= $products[1] ?></h2>
					<p class="p"><?= $products[2] ?></p>
					<p class="price">От <?= $products[3] ?> Р</p>
					<button><a href="detail_monitor.php?id=<?= $products[0] ?>">Подробнее</a></button>
				</div>
