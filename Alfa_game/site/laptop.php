...
<body>
	<div class="main_grid">
		...
		<main>
			<div class="txt">
				<h1>Игровые ноутбуки</h1>
				<p>Игровые ноутбуки с индивидуальной сборкой, которые создаются<br> специально для вас, под любой бюджет и с лучшей ценой. Используем<br> только надежные комплектующие, гарантируем качество нашей<br> продукции и предоставляем техническую поддержку.<br>Каждый ноутбук имеет конфигурации на выбор, а также конфигуратор с<br> проверкой совместимости – вы легко можете настроить сборку под свои предпочтения.<br>Мы обеспечиваем быструю и надежную доставку вашего нового ноутбука<br> в любую точку страны.</p>
			</div>
			...
			<a name="comps"><h2>Модели игровых ноутбуков</h2></a>

			<div class="parent">
			<?php

				$products = mysqli_query($link, query:"SELECT *  FROM `products` WHERE `category_id` = '2'"); /* Выборка элементов из базы данных */
				$products = mysqli_fetch_all($products);
				foreach ($products as $products) {
				?>
				<div class="div">
					<h3>GeForce GTX 1650</h3>
					<p>Игровой ноутбук на</p>
					<img class="img" src="css/img_laptop/<?= $products[8] ?>.png" alt="">

					<h2><?= $products[1] ?></h2>
					<p class="p"><?= $products[2] ?></p>
					<p class="price">От <?= $products[3] ?> Р</p>
					<button><a href="detail_laptop.php?id=<?= $products[0] ?>">Подробнее</a></button>
				</div>
